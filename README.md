...
Step-by-step guide for preparing JW congregation territories using Google My Maps.

## Overview

Google My Maps allows you to draw custom maps and export a link to each of these maps. Clicking that link displays the custom map directly in the Google Maps app.

There is no theoretical limit to the number of custom maps that can live in your Google Drive (which serves as storage for the maps you create in Google My Maps).

You can create these maps one by one without any problems.

The goal of this repository is to guide you through creating a single master map in Google My Maps, splitting the shapes (territories) in it into separate CSV files, and importing them back to create separate territory maps.
...
2. Create a new map.
3. Draw your territories (you can use up to 10 layers). For best results, each territory should have a `name` field.
...
> **Tip:** Keep all exported CSV files in a single folder. The script will process every CSV file it finds in that folder.
...
4. On line 3, replace `YOUR_SOURCE_FOLDER_ID_HERE` with the Google Drive folder ID from Step 2.
5. Save the project.
...
- Places all the generated CSV files in a subfolder called `Split Maps Output` inside the source folder.

## Step 4 – Import each split CSV back into Google My Maps

1. Open [Google My Maps](https://www.google.com/mymaps).
2. Create a new map for each territory.
3. In the new map, click **Import** and select one of the CSV files from the `Split Maps Output` folder.
4. Repeat this process for each split CSV file.
